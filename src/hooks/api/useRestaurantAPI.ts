...
export const useRestaurantAPI = () => {
  ...
  const searchRestaurants = async (params: RestaurantSearchParams) => {
    ...
    try {
      // Call RapidAPI
      const url = new URL('https://travel-advisor.p.rapidapi.com/restaurants/list-by-latlng');
      ...
      const processedRestaurants = data.data
        .filter((place: any) => {
          ...
          const nonRestaurantKeywords = [
            'museum', 'gallery', 'theater', 'cinema', 'park', 'garden', 
            'monument', 'statue', 'tower', 'bridge', 'castle', 'palace',
            'temple', 'church', 'mosque', 'synagogue', 'shrine', 'attraction',
            'landmark', 'viewpoint', 'observation', 'tour', 'experience',
            'adventure', 'entertainment', 'amusement', 'zoo', 'aquarium',
            'hotel', 'hostel', 'resort', 'spa', 'gym', 'fitness'
          ];
          
          const isNotRestaurant = nonRestaurantKeywords.some(keyword => 
